 ...
 * The number of Tasks that can be simultaneously executed is determined by
 * the size of the pool. In addition, the TaskExecutorPool will optionally
 * allow the thread that is calling it's doTask() function to block while
 * awaiting a TaskExecutor to become available.
 * <P>
 * A TaskExecutorPool may optionally be given a name, a number of
 * TaskExecutors to always retain in it's pool, a maximum number of
 * TaskExecutors that may execute Tasks simultaneously, and the stack size to
 * use when creating TaskExecutor threads.
 * 
...
public final class TaskExecutorPool {
    ...
    public static TaskExecutorPool getPool() {
        ...
        if (log.isLoggable(Level.FINER)) {
            log.exiting("com.InfoMontage.task.TaskExecutorPool",
                "getPool()", "end of method - return value = " + ttf);
        }
        return ttf;
    }
        ...
        int retainedPoolSize, int absoluteMaxThreads)
    {
        if (log.isLoggable(Level.FINER)) {
            log.entering("com.InfoMontage.task.TaskExecutorPool",
                "getPool(String name = " + name
                    + ", int retainedPoolSize = " + retainedPoolSize
                    + ", int absoluteMaxThreads = " + absoluteMaxThreads
                    + ")", "start of method");
        }

        TaskExecutorPool ttf = new TaskExecutorPool(name);
        initializeTaskExecutorPool(ttf, retainedPoolSize,
            absoluteMaxThreads, true,
    ...
    public static TaskExecutorPool getPool(String name,
        int retainedPoolSize, int absoluteMaxThreads, boolean allowBlocking)
    {
        if (log.isLoggable(Level.FINER)) {
            log.entering("com.InfoMontage.task.TaskExecutorPool",
                "getPool(String name = " + name
                    + ", int retainedPoolSize = " + retainedPoolSize
                    + ", int absoluteMaxThreads = " + absoluteMaxThreads
                    + ", boolean allowBlocking = " + allowBlocking + ")",
                "start of method");
        }

        TaskExecutorPool ttf = new TaskExecutorPool(name);
        initializeTaskExecutorPool(ttf, retainedPoolSize,
            absoluteMaxThreads, allowBlocking,
            Defaults.DEFAULT_TASK_EXECUTOR_THREAD_STACK_SIZE,
            Defaults.DEFAULT_TASK_QUEUE_START_CAPACITY,
            Defaults.DEFAULT_TASK_QUEUE_CAPACITY_INCREMENT,
            Defaults.DEFAULT_TASK_QUEUE_MAXIMUM_CAPACITY);
    ...
    public static TaskExecutorPool getPool(String name,
        int retainedPoolSize, int absoluteMaxThreads,
        boolean allowBlocking, int threadStackSize)
    {
        if (log.isLoggable(Level.FINER)) {
            log.entering("com.InfoMontage.task.TaskExecutorPool",
                "getPool(String name = " + name
        ...
        TaskExecutorPool ttf = new TaskExecutorPool(name);
        initializeTaskExecutorPool(ttf, retainedPoolSize,
            absoluteMaxThreads, allowBlocking, threadStackSize,
            Defaults.DEFAULT_TASK_QUEUE_START_CAPACITY,
            Defaults.DEFAULT_TASK_QUEUE_CAPACITY_INCREMENT,
            Defaults.DEFAULT_TASK_QUEUE_MAXIMUM_CAPACITY);
     ...
     * @throws OutOfMemoryError TBD: do
     */
        ...
        if (log.isLoggable(Level.FINER)) {
            log.entering("com.InfoMontage.task.TaskExecutorPool",
                "initializeTaskExecutorPool(TaskExecutorPool ttf = " + ttf
                    ...
                    + ", boolean allowBlocking = " + allowBlocking
                    + ", int threadStackSize = " + threadStackSize
                    + ", int taskQueueCapacity = " + taskQueueCapacity
     ...
     * @throws InterruptedException if the thread calling this function is
     *             interrupted while waiting for the TaskExecutorPool's
     *             blocking task executor thread. This will only happen if
        ...
        if (log.isLoggable(Level.FINER)) {
            log
                .exiting("com.InfoMontage.task.TaskExecutorPool",
                    "getTaskExecutor()", "end of method - return value = "
                        + rt);
        }
        return rt;
    }
