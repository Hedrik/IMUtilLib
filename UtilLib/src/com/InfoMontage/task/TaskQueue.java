...
public class TaskQueue {
    ...
    boolean enqueue(Task t) {
        if (log.isLoggable(Level.FINER)) {
            log.entering("com.InfoMontage.task.TaskQueue",
                "enqueue(Task t = " + t + ")", "start of method");
        }
        ...
        throws InterruptedException
    {
        if (log.isLoggable(Level.FINER)) {
            log.entering("com.InfoMontage.task.TaskQueue",
                "enqueue(Task t = " + t + ", boolean blocking = "
                    + blocking + ")", "start of method");
        }
        ...
        if (log.isLoggable(Level.FINER)) {
            log.exiting("com.InfoMontage.task.TaskQueue",
                "enqueue(Task t = " + t + ", boolean blocking = "
                    + blocking + ")", "end of method - return value = "
                    + queued);
        }
        return queued;
    }
    ...
    int getMaxQueueSize() {
        if (log.isLoggable(Level.FINER)) {
            log.entering("com.InfoMontage.task.TaskQueue",
                "getMaxQueueSize()", "start of method");
        }

        if (log.isLoggable(Level.FINER)) {
            log.exiting("com.InfoMontage.task.TaskQueue",
                "getMaxQueueSize()", "end of method - return value = "
                    + maxQueueSize);
        }
        return maxQueueSize;
    }
